class Partita {
  ...
  constructor(player1, player2, mazzo) {
    ...
    this.startRound();
    this.setKeepalive(this.#players);
  }

  setKeepalive(players) {
    setInterval(() => {
      players.forEach((p) => {
  ...
  startRound() {
    ...
    this.#players.forEach((socket, index) => {
      socket.on("message", (message) => {
        ...
        if (data.type === "move") {
          console.log("Prima della mossa effettiva: " + JSON.stringify(this.#tableCards));
          ...
          console.log("Dopo il managePresa: " + JSON.stringify(this.#tableCards));
          ...
          if (this.#players[opponentIndex]) {
            ...
            if (take.taken == true) {
              this.#lastToGetCards = this.#players[playerIndex]; //checkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk
              if (take.combosAvail) {
          ...
          if (
            this.#hands[0].length === 0 &&
            this.#hands[1].length === 0 &&
            ...
            this.assignScore(); //aggiunge punti ai player dentro l'array points[]
            if (this.#points[0] > 11 || this.#points[1] > 11) {
              ...
              this.#mazzo.shuffle();
              this.#players.forEach((p) => {
                p.send(JSON.stringify({type: "matchEnd"}));
              })
              this.dealTableCards();
  ...
  gestisciUltimeCarte() {
    ...
    let playerIndex = this.#lastToGetCards;
  ...
  assignScore() {
    ...
    console.log(
      "player 1: " + this.#points[0] + " player 2: " + this.#points[1]
    );
  }
  ...
  managePresa(playedCard, playerIndex) {
    ...
    if (sameValueCard) {
      console.log(
        "C'è una carta diretta: " + sameValueCard.valore + sameValueCard.seme
      );
